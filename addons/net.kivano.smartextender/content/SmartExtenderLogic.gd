...
const TEMPLATE_SCN_FILEPATH               = "res://addons/net.kivano.smartextender/content/templates/template_inherited_scene";
const TEMPLATE_SCN_SCRIPTED_FILEPATH      = "res://addons/net.kivano.smartextender/content/templates/template_inherited_script_scene";
const TEMPLATE_SCN_SIMPLE_SCRIPT_FILEPATH = "res://addons/net.kivano.smartextender/content/templates/template_simple_script_scene";
const TEMPLATE_SCRIPT_FILEPATH            = "res://addons/net.kivano.smartextender/content/templates/template_inherited_script";
const TEMPLATE_USER_SCRIPT_FILEPATH       = "res://addons/net.kivano.smartextender/SCRIPT_TEMPLATE";
...
func _onUserNodeSelectionChanged():
	var selectedNodes = editorPlugin.get_selection().get_selected_nodes();
...
func createSceneFromDialog():
	...
	var selectedNode = editorPlugin.get_selection().get_selected_nodes()[0];
	...
	if(newSceneFilepath.extension()=="gd"):
		newSceneFilepath = newSceneFilepath.replace(".gd",".tscn");
	
	if(newSceneFilepath.extension()!="tscn") && (newSceneFilepath.extension()!="scn"):
		if(newSceneFilepath.extension()==null): 
	...
	if(selectedNode.get_script()!=null):
		newScriptFilepath = newSceneFilepath.replace(newSceneFilepath.extension(), "gd");
	...
	if(selectedNode.get_script()==null): 
		...
		scriptType = selectedNode.get_type();
	else: 
...
func saveExtendedScriptOnDisk(inBaseScnNode, inNewSceneFilepath):
	...
	var extendedScriptFilepath = inNewSceneFilepath.replace(inNewSceneFilepath.extension(), "gd");
